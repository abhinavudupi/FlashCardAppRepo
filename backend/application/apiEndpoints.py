...
class SignupAPI(Resource):
    ...
    def post(self):
        try:
            ...
            db.session.commit()
            with open("./templates/welcome_mail.html") as fil:
                template = Template(fil.read())
            mail_body = template.render(data={'username':un})
            job = tasks.send_mail.delay(to_addr=em, subject='Welcome to Flash Card App', body=mail_body)
...
class DeckResource(Resource):
    ...
    def get(self, deck_id):  # get deck by deck id
        try:
            ...
            try:
                if cacheable.isAuth(user_id, request.headers['APItoken']):
                    pass
                else:
            ...
            for card in cardsInDeck:
                ...
                b = card.back
                cs = card.card_score
                tempDict = {'card_id': ci, 'front': f,
                            'back': b, 'card_score': cs}
                cards.append(tempDict)
            td1 = {'deck_id': di, 'user_id': ui, 'deck_name': dn,
                   'total_score': ts, 'last_reviewed': str(lr), 'cards': cards}
    ...
    def put(self, deck_id):  # update deck by deck id
        try:
            ...
            if not currentDeck:
                return{'status': 'error', 'message': 'deck not present'}
            currentDeck.deck_name = str(json_values.get(
...
class UserdeckResource(Resource):
    ...
    def get(self, user_id):  # get all decks by user id
        try:
            ...
            for deck in decks:
                di = deck.deck_id
                tempres = requests.get('http://localhost:8080/api/decks/'+str(di)+'?user_id='+str(
                    user_id), headers={"APItoken": str(request.headers['APItoken'])})
                # tempres = {}
...
class DeckallResource(Resource):
    ...
    def post(self):  # create new deck
        try:
            ...
            db.session.commit()
            return {'status': 'success', 'message': 'successfully created', 'deck_id':newDeck.deck_id}
        except:
            abort(500)
...
def updateDeckScore(deck_id, uid,  apit):
    totS = 0
    cards = requests.get('http://localhost:8080/api/decks/cards/' +
                         str(deck_id)+"?user_id="+str(uid), headers={'APItoken': apit})
    cards = cards.json()
    cards = cards['data']
    for card in cards:
        totS += card['card_score']
    update = requests.put('http://localhost:8080/api/decks/'+str(deck_id),
                          json={'total_score': totS, 'last_reviewed': 'now'})
...
class DeckcardResource(Resource):
    ...
    def get(self, deck_id):  # get card by deck id
        try:
            ...
            for currentCard in currentDeck:
                ci = currentCard.card_id
                tempres = requests.get(
                    'http://localhost:8080/api/cards/'+str(ci))
...
class ExportDeckResource(Resource):
    ...
    def get(self, deck_id):
        ...
        try:
            ...
            return {'status': 'error', 'message': 'no API token'}
        job = tasks.export_deck.delay(
            deck_id, user_id, request.headers['APItoken'])
...
class ImportDeckResource(Resource):
    ...
    def post(self):
        ...
        if 'files[]' in request.files:
            ...
            for f in files:
                f.save("imports/"+f.filename)
                jobs.append((tasks.import_deck.delay(
                    f.filename, user_id, request.headers['APItoken'])).__str__())
        return {'status': 'success', 'message': jobs}
...
class UserResource(Resource):
    ...
    def put(self, user_id):  # update user details
        try:
            ...
            if not currentUser:
                return{'status': 'error', 'message': 'User not present'}
            currentUser.username = json_values.get(
                'username', currentUser.username)
            currentUser.password = json_values.get(
    ...
    def delete(self, user_id):  # delete user
        try:
            ...
            for deck in userdecks:
                di = deck.deck_id
                tempres = requests.delete(
                    'http://localhost:8080/api/decks/'+str(di))
...
api.add_resource(ExportedResource, '/api/exports/<string:file_name>')
